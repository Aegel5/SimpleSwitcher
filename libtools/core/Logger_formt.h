...
template<class ...T> inline void SW_LOG_INFO(const TChar* Format, T&&... v)
{
	std::unique_lock<std::mutex> _lock(SwLoggerGlobal().Mtx());

	__SW_LOG_TIME();
	SwLoggerGlobal().Append(std::format(Format, v...).c_str());
...
inline void __Log_Err_Common(T err, const char* file, int line, const wchar_t* format = nullptr, TArgs&&... args)
{
	...
	SW_LOG_INFO(L" {}({}) ", file,line);
    
	__SW_LOG_TIME();
	err.Log();
	SW_LOG_INFO(L" {}({}) ", file, line);
	if(format){
		SW_LOG_INFO(format, args);
	}
	SwLoggerGlobal().EndLineFlash();
}
