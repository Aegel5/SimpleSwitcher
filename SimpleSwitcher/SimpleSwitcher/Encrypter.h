...
public:
    static TStatus Encrypt(TKeyHookInfo& key) {
		...
		IFW_RET(CryptProtectMemory(&key.crypted, CRYPTPROTECTMEMORY_BLOCK_SIZE, CRYPTPROTECTMEMORY_SAME_PROCESS));
		RETURN_SUCCESS;
    }
    static TStatus Decrypt(TKeyHookInfo& key) {
		IFW_RET(CryptUnprotectMemory(&key.crypted, CRYPTPROTECTMEMORY_BLOCK_SIZE, CRYPTPROTECTMEMORY_SAME_PROCESS));
		RETURN_SUCCESS;
    }
};