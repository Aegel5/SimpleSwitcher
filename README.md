...
Simple Switcher - переключатель клавиатуры, программа для быстрого исправления языка набранного текста в Windows. Полностью открыт и бесплатен, написан на современном `С++ 23` (по большей части 🤦‍♂️), турбо-реактивен 🛴 и никак не сказывается на работе ОС. Список фич небольшой, но он постоянно пополняется, и при этом каждая фича прорабатывается, чтобы быть простой и надежной.
