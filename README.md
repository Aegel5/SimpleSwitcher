...
- [Ночные сборки](https://github.com/Aegel5/SimpleSwitcher/actions) (рекомендуется)
...
_Способ 2. F24:_
Использование клавиши F24 полностью убирает проблему случайного включкение режима CapsLock, даже если SS не запущен. Для активации CapsLock используется специальный хоткей.
...
_Способ 3. CaspsLock/F24_:
Самый универсальный. Назначить оба хоткея на требуемую функцию. Переключение будет работать как в случае если CapsLock был переназначен, так и не был.

## Альтернативный режим переключения (эмуляция Alt+Shift)
Если основной режим работает некорректно (https://github.com/Aegel5/SimpleSwitcher/issues/83), то можно использовать альтернативный режим.
...
Есть вопрос, предложение или найден баг - добро пожаловать в [Issues](https://github.com/Aegel5/SimpleSwitcher/issues). Issues c тегом `discussion` служат для обсуждения. **Важно:** если проблема уже есть в списке, то лучше все равно оставить комментарий, чтобы показать, что проблема актуальна.
...
<img width="200" height="200" alt="qr_код" src="https://github.com/user-attachments/assets/4418c72d-7cf7-42e1-80fd-c4e5dfe29a0c" />
